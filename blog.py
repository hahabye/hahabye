...
def fetch_feed(url: str) -> list[dict[str, str]]:
    try:
        ...
        if not feed.entries:
            logging.error("Empty feed: no entries found")
            return []
...
def format_entry_date(entry: Any, date_format: str = DEFAULT_DATE_FORMAT) -> str:
    if hasattr(entry, "published_parsed"):
        ...
        # Define the date format according to your needs
        df = '%Y-%m-%dT%H:%M:%S%z'
        published_time = datetime.strptime(entry.updated, df)
        # Format it as needed
        return published_time.strftime(date_format)
    return ""
...
if __name__ == "__main__":
    ...
    url = "https://blog.196000.xyz/atom.xml"
